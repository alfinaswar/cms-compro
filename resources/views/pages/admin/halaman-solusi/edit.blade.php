...
@section('content')
    ...
    <section class="content">
        <form action="{{ route('halaman-solusi.update', encrypt($data->id)) }}" method="POST" enctype="multipart/form-data"
            id="formSolusi">
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="Judul" id="Judul"
                                    ...
                                    placeholder="Tulis judul solusi..." value="{{ old('Judul', $data->Judul) }}" required
                                    autofocus>
                                @error('Judul')
                            ...
                            <div class="form-group">
                                <label for="DeskripsiSingkat"><strong>Deskripsi Singkat</strong></label>
                                <textarea name="DeskripsiSingkat" id="DeskripsiSingkat"
                                    ...
                                    rows="3">{{ old('DeskripsiSingkat', $data->DeskripsiSingkat) }}</textarea>
                    ...
                    <div class="card shadow-sm border-0">
                        ...
                        <div class="card-body">
                            <textarea name="Konten" id="summernote" class="form-control @error('Konten') is-invalid @enderror" rows="8">{{ old('Konten', $data->Konten) }}</textarea>
                            @error('Konten')
                                <span class="invalid-feedback d-block mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="card-body">
                            <div id="detail-solusi-wrapper">
                                @php
                                    ...
                                    if (!$details) {
                                        $details = [];
                                        if (isset($data->getSolusiDetail) && count($data->getSolusiDetail) > 0) {
                                            foreach ($data->getSolusiDetail as $item) {
                                                $details[] = [
                                                    'judul' => $item->Judul,
                                                    'gambar' => $item->Gambar,
                                            ...
                                            $details[] = ['judul' => '', 'gambar' => '', 'keterangan' => ''];
                                        }
                                    }
                                @endphp
                                            ...
                                            <div class="col-md-5">
                                                <div class="form-group mb-2">
                                                    ...
                                                    <input type="text" name="detail[{{ $i }}][judul]"
                                                        class="form-control"
                                                        value="{{ old('detail.' . $i . '.judul', is_array($detail) ? $detail['judul'] ?? '' : $detail->Judul ?? '') }}"
                                                        required>
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                                <div class="form-group mb-2">
                                                    ...
                                                    <div class="custom-file">
                                                        <input type="file" name="detail[{{ $i }}][gambar]"
                                                            ...
                                                            id="gambar-{{ $i }}">
                                                        <label class="custom-file-label" for="gambar-{{ $i }}">
                                                            @php
                                                                $gambar = is_array($detail)
                                                                    ? $detail['gambar'] ?? ''
                                                                    : $detail->Gambar ?? '';
                                                            @endphp
                                                            {{ $gambar ? $gambar : 'Pilih file' }}
                                                        </label>
                                                        @if (!empty($gambar))
                                                            <div class="mt-1">
                                                                {{-- Preview gambar lama --}}
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group mb-2">
                                                    ...
                                                    <input type="text" name="detail[{{ $i }}][keterangan]"
                                                        class="form-control"
                                                        value="{{ old('detail.' . $i . '.keterangan', is_array($detail) ? $detail['keterangan'] ?? '' : $detail->Keterangan ?? '') }}">
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="card shadow-sm border-0">
                        ...
                        <div class="card-body">
                            ...
                            <small class="text-muted d-block mt-2">
                                Disarankan ukuran 312×240 px.<br>
                                Gambar akan ditampilkan secara optimal jika berukuran tepat 312x240 piksel.
                            </small>
                            @if (!empty($data->Thumbnail))
                                <div class="mt-3">
                                    <img src="{{ asset('storage/' . $data->Thumbnail) }}" class="img-thumbnail"
                                        style="max-width:312px; max-height:240px; width:312px; height:240px; object-fit:cover;"
                                        alt="Thumbnail Lama">
                                    <small class="text-muted d-block mt-1">Preview gambar cover saat ini (312x240).</small>
                                </div>
                            @endif
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select name="IsPublished" id="IsPublished" class="form-control">
                                    <option value="0"
                                        {{ old('IsPublished', $data->IsPublished) == '0' ? 'selected' : '' }}>Draft
                                        (Tidak Dipublikasikan)</option>
                                    <option value="1"
                                        {{ old('IsPublished', $data->IsPublished) == '1' ? 'selected' : '' }}>
                                        Publikasikan
                                    </option>
                                </select>
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="SEOTitle" id="inputSEOTitle" class="form-control"
                                    maxlength="70" value="{{ old('SEOTitle', $data->SEOTitle) }}"
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="SEODescription" id="inputSEODesc" class="form-control" rows="3" maxlength="160"
                                    placeholder="Deskripsi untuk hasil pencarian Google">{{ old('SEODescription', $data->SEODescription) }}</textarea>
                            </div>

                            <div class="form-group mb-0">
                                <label><strong>SEO Keywords</strong></label>
                                <input type="text" name="SEOKeywords" class="form-control"
                                    value="{{ old('SEOKeywords', $data->SEOKeywords) }}"
                                    placeholder="solusi, inovasi, keyword...">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Preview Thumbnail (jika ada field Thumbnail)
