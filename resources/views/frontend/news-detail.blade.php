...
@section('content-frontend')
    @php
        // Helper untuk berita utama
        $trans = $news->translate($locale);
        $displayJudul = $trans->Judul ?: $news->Judul;
        $displayKonten = $trans->Konten ?: $news->Konten;
        $thumbnailUrl = $news->PathThumbnail ? asset('storage/' . $news->PathThumbnail) : asset('img/no-image.png');
    @endphp
        ...
        <div class="container mx-auto px-6 relative z-10">
            <div class="max-w-4xl mx-auto text-center">
                ...
                <nav class="mt-8 flex items-center justify-center space-x-2 text-sm text-slate-300">
                    ...
                    <a href="{{ url('news') }}" class="hover:text-white transition-colors">{{ __('News') }}</a>
                    <i class="fa-solid fa-chevron-right text-xs text-slate-500"></i>
                    <span class="text-white font-semibold">{{ Str::limit($displayJudul, 30) }}</span>
                </nav>
            </div>
        </div>
    </section>
                ...
                <div class="lg:col-span-2">
                        ...
                        <div class="p-6 md:p-8">
                            ...
                            <div class="flex flex-wrap items-center gap-4 text-sm text-slate-500 mb-4">
                                ...
                                <span>
                                    ...
                                    {{ $news->TanggalPublikasi ? \Carbon\Carbon::parse($news->TanggalPublikasi)->isoFormat('D MMM, Y') : '-' }}
                                </span>
                                <a href="{{ url('news?kategori=' . urlencode($news->Kategori)) }}"
                            ...
                            @if ($news->Tags)
                                ...
                                <div class="border-t border-slate-200 pt-6 mt-8">
                                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                        <!-- Tags -->
                                        <div>
                                            <span class="text-sm font-semibold text-slate-700 mr-2">{{ __('Popular Tags') }}:</span>
                                        ...
                                        <div>
                                            ...
                                            <div class="inline-flex gap-2 mt-2">
                                                ...
                                                @endphp
                                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ $shareUrl }}" target="_blank" rel="noopener" class="w-9 h-9 bg-slate-100 hover:bg-blue-600 hover:text-white text-slate-600 rounded-lg flex items-center justify-center transition-colors">
                                                    <i class="fa-brands fa-facebook-f"></i>
                                                </a>
                                                <a href="https://twitter.com/intent/tweet?url={{ $shareUrl }}&text={{ $shareTitle }}" target="_blank" rel="noopener" class="w-9 h-9 bg-slate-100 hover:bg-sky-500 hover:text-white text-slate-600 rounded-lg flex items-center justify-center transition-colors">
                                                    <i class="fa-brands fa-twitter"></i>
                                                </a>
                                                <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ $shareUrl }}" target="_blank" rel="noopener" class="w-9 h-9 bg-slate-100 hover:bg-blue-700 hover:text-white text-slate-600 rounded-lg flex items-center justify-center transition-colors">
                                                    <i class="fa-brands fa-linkedin-in"></i>
                                                </a>
                                                <a href="https://wa.me/?text={{ $shareTitle }}%20{{ $shareUrl }}" target="_blank" rel="noopener" class="w-9 h-9 bg-slate-100 hover:bg-green-600 hover:text-white text-slate-600 rounded-lg flex items-center justify-center transition-colors">
                                                    <i class="fa-brands fa-whatsapp"></i>
                                                </a>
                                                <button onclick="copyLink()" id="copyLinkBtn" class="w-9 h-9 bg-slate-100 hover:bg-brand-600 hover:text-white text-slate-600 rounded-lg flex items-center justify-center transition-colors">
                    ...
                    @if ($prevPost || $nextPost)
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                            @if ($prevPost)
                                @php $prevTrans = $prevPost->translate($locale); $prevJudul = $prevTrans->Judul ?: $prevPost->Judul; @endphp
                                <a href="{{ url('news/' . $prevPost->Slug) }}" class="bg-white rounded-xl p-5 border border-slate-200 hover:border-brand-500 hover:shadow-md transition-all group">
                                    <div class="flex items-center">
                                        <i class="fa-solid fa-arrow-left text-brand-600 mr-3 group-hover:-translate-x-1 transition-transform"></i>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-xs text-slate-500 mb-1">Previous Post</p>
                                            <h4 class="text-sm font-semibold text-slate-900 line-clamp-2 group-hover:text-brand-600">
                                                {{ Str::limit($prevJudul, 40) }}
                                            </h4>
                                        </div>
                                    </div>
                                </a>
                            ...
                            @if ($nextPost)
                                @php $nextTrans = $nextPost->translate($locale); $nextJudul = $nextTrans->Judul ?: $nextPost->Judul; @endphp
                                <a href="{{ url('news/' . $nextPost->Slug) }}" class="bg-white rounded-xl p-5 border border-slate-200 hover:border-brand-500 hover:shadow-md transition-all group">
                                    <div class="flex items-center text-right md:flex-row-reverse">
                                        <i class="fa-solid fa-arrow-right text-brand-600 ml-3 group-hover:translate-x-1 transition-transform"></i>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-xs text-slate-500 mb-1">Next Post</p>
                                            <h4 class="text-sm font-semibold text-slate-900 line-clamp-2 group-hover:text-brand-600">
                                                {{ Str::limit($nextJudul, 40) }}
                                            </h4>
                                        </div>
                                    </div>
                                </a>
                    ...
                    <div class="bg-white rounded-xl p-6 border border-slate-200 mt-6">
                        <div class="flex items-start gap-4">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($news->Penulis ?? 'Admin') }}&background=0284c7&color=fff" alt="Author" class="w-16 h-16 rounded-full object-cover flex-shrink-0">
                            <div class="flex-1">
                    ...
                    @if ($relatedPosts->count() > 0)
                        <div class="mt-8">
                            <h3 class="text-2xl font-bold text-slate-900 mb-6">{{ __('Recent Posts') }}</h3>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                @foreach ($relatedPosts as $related)
                                    @php
                                        $relTrans = $related->translate($locale);
                                        $relJudul = $relTrans->Judul ?: $related->Judul;
                                        $relThumb = $related->PathThumbnail ? asset('storage/' . $related->PathThumbnail) : asset('img/no-image.png');
                                    @endphp
                                    <article class="bg-white rounded-xl overflow-hidden border border-slate-200 hover:shadow-md transition-shadow">
                                        <a href="{{ url('news/' . $related->Slug) }}" class="block overflow-hidden">
                                            <img src="{{ $relThumb }}" alt="{{ $relJudul }}" class="w-full h-40 object-cover hover:scale-105 transition-transform duration-300">
                                        </a>
                                        <div class="p-4">
                                            <p class="text-xs text-slate-500 mb-2">
                                                <i class="fa-regular fa-calendar mr-1"></i>
                                                {{ $related->TanggalPublikasi ? \Carbon\Carbon::parse($related->TanggalPublikasi)->isoFormat('D MMM, Y') : '-' }}
                                            </p>
                                            <h4 class="text-sm font-bold text-slate-900 hover:text-brand-600 line-clamp-2">
                                                <a href="{{ url('news/' . $related->Slug) }}">
                                                    {{ Str::limit($relJudul, 50) }}
                                                </a>
                                            </h4>
                                        </div>
                                    </article>
